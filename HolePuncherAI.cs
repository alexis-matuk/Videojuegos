...
public class HolePuncherAI : MonoBehaviour {

	Transform target; //Aphelion
	float moveSpeed = 500; //velocidad
	float rotationSpeed;
	...
	void Start()
	{
		...
		rotationSpeed = GameObject.FindGameObjectWithTag("Player").GetComponent<Aphelion>().getSpeed()*.005f;
	...
	void Update () {
		...
		GetComponent<Rigidbody2D>().AddRelativeForce(new Vector2(0,moveSpeed));//moverse hacia adelante
