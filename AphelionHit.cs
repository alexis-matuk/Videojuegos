...
public class AphelionHit : MonoBehaviour {
	...
	void OnTriggerStay2D(Collider2D col)
	{
		...
		if(inside && col.gameObject.tag == "Bomb")
		{
			Bomb bomb = col.gameObject.GetComponent<Bomb>();
			if(bomb && !onCD)
			{
				hitByBomb(bomb, aphelion, col);
				if(shieldDone)
					gameObject.GetComponent<BulletFireScript>().resetBullet();//quitar power up de aphelion
			}
		}
		
		EliteMissile miss = col.GetComponent<EliteMissile>();
		if(miss && !onCD)//si fue un enemigo
		{
			hitByEliteMissile(miss, aphelion, col);
			if(shieldDone)
				gameObject.GetComponent<BulletFireScript>().resetBullet();//quitar power up de aphelion
		}
		
		EliteBomber bomber = col.GetComponent<EliteBomber>();
		if(bomber && !onCD)//si fue un enemigo
		{
			hitByEliteBomber(bomber, aphelion, col);
			if(shieldDone)
				gameObject.GetComponent<BulletFireScript>().resetBullet();//quitar power up de aphelion
		}
	}
	
	public void insideBomb()
	{
		inside = true;
	}
...
//		hpBar.fillAmount -= convertDamage(bull.getDamage(), aphelion.getInitialHealth());//reducir hp de barra
	...
	void hitByBomb(Bomb bomb, Aphelion aphelion,Collider2D col)
	{
		aphelion.reduceHealthOrShield(bomb.getDamage());//reducir hp de aphelion
		SpriteRenderer sprite =  gameObject.GetComponent<SpriteRenderer>();
		if(shieldDone)
		{
			sprite.color = new Color(1f,1f,1f,.5f);//cambiar sprite cuando lo golpean
		...
		StartCoroutine(coolDownDmg(sprite));//funcion de cooldown
		Instantiate (bomb.getExplosion(), bomb.transform.position, bomb.transform.rotation);//instanciar explosión
		inside = false;
	}
	
	void hitByEliteMissile(EliteMissile miss, Aphelion aphelion, Collider2D col)
	{
		aphelion.reduceHealthOrShield(miss.getDamage());//reducir hp de aphelion
		SpriteRenderer sprite =  gameObject.GetComponent<SpriteRenderer>();
		if(shieldDone)
		{
			sprite.color = new Color(1f,1f,1f,.5f);//cambiar sprite cuando lo golpean
		...
		Instantiate (miss.getExplosion(), miss.transform.position, miss.transform.rotation);//instanciar explosión
	}
	
	void hitByEliteBomber (EliteBomber bomber, Aphelion aphelion, Collider2D col)
	{
		aphelion.reduceHealthOrShield(bomber.getContactDamage());//reducir hp de aphelion
		SpriteRenderer sprite =  gameObject.GetComponent<SpriteRenderer>();
		if(shieldDone)
		{
			sprite.color = new Color(1f,1f,1f,.5f);//cambiar sprite cuando lo golpean
